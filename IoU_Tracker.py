...
# compute center point of a bouding box
def get_center(box):
    x1, y1, x2, y2 = box
    center_x = (x2 + x1)/2
    center_y = (y2 + y1)/2 
...
class tracklet:
    ...
    def get_pose(self):
        self.pose = [get_center(b) for b in self.boxes]
...
class tracker:
    def __init__(self):
        ...
        # total number of consecutive misses before deletion
        self.max_miss = 900
    ...
    def run(self, detections, features):
            ...
            if len(self.cur_tracklets) == 0:
                ...
                for i in range(len(confirmed_tracklets)):
                    for j in range(len(cur_frame_detection)):
                        # Compute cost based on embedding features and mahalonbios distance
                ...
                # get unmatched detections, features and tracklets after matching confirmed trackelts
                ...
                # combine unconfirmed tracklets and unmatched trackelts and match with remaining detection
                ...
                # update macthced trackelts
                for idx in range(matches):
                    row,col = row_inds[idx],col_inds[idx]
                    # if there is only 1 trackelts and detection and the iou score is low then update
                    # otherwise consider a miss and initialize new tracklets
        ...
        # calculate center position of all bounding boxes
        for trk_id in range(len(final_tracklets)):
            final_tracklets[trk_id].get_avg_features()
            final_tracklets[trk_id].get_pose()

        return final_tracklets
    
    # initialize tracklets and associated Kalman Filter
